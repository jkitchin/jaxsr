...
def plot_pareto_front(
    ...
    ax.plot(
        [complexities[i] for i in sorted_idx], [mse_values[i] for i in sorted_idx], "b--", alpha=0.3
    )
...
def plot_prediction_surface(
    ...
    # Plot surface
    ax.plot_surface(X1, X2, np.array(Y), cmap="viridis", alpha=0.8)
...
def plot_comparison(
    ...
    ax = axes[2]
    for i, (_model, name) in enumerate(zip(models, names, strict=False)):
        ax.scatter(metrics[i]["complexity"], metrics[i]["mse"], s=150, label=name)
    ax.set_xlabel("Complexity")
    ax.set_ylabel("MSE")
...
def plot_prediction_intervals(
    ...
    ax.fill_between(
        ...
        alpha=0.15,
        color="steelblue",
        label=f"{int((1-alpha)*100)}% Prediction",
    )
    ...
    ax.fill_between(
        ...
        alpha=0.3,
        color="steelblue",
        label=f"{int((1-alpha)*100)}% Confidence",
    )
    # Mean prediction
...
def plot_coefficient_intervals(
    ...
    y_pos = np.arange(len(names))
    errors = np.array(
        [[est - lo, hi - est] for est, lo, hi in zip(estimates, lowers, uppers, strict=False)]
    ).T
